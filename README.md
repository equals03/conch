...
If you want the generated file to ignore repeated sourcing in the same shell session, enable a top-level init guard:
...
Optional top-level init guards can also be enabled:
...
This is an excerpt from `conch init fish --config examples/conch.toml` — your generated file will include every block from your config, in resolved order. If `[init.guard] enabled = true`, conch adds a shell-specific sourced guard before these blocks:
...
| `reserved env key __CONCH_...` | `[init.guard] enabled = true` reserves `__CONCH_SOURCED`, `__CONCH_FISH_SOURCED`, and `__CONCH_BASH_SOURCED`. Rename the env binding or disable the init guard. |
