...
> [!IMPORTANT]
> `conch` does **not** evaluate `when` / `requires` during `check`, `build`, or `explain`. It validates predicates, orders blocks, detects conflicts, and emits **guarded shell** that your shell evaluates when you `source` the generated file.
...
- `check`, `build`, and `explain` commands
...
# or YAML / JSON:
conch build fish --config examples/conch.yaml > ~/.config/conch/config.fish
```

```bash
conch build bash --config examples/conch.toml > ~/.config/conch/config.bash
conch build bash --config examples/conch.json > ~/.config/conch/config.bash
```
...
If you prefer to run `conch` on each login so the shell always loads freshly compiled output (at the cost of startup time), call `build` from your rc file and feed it straight into the shell. Use the **bash** output in bash and the **fish** output in fish.

**Fish** — in `~/.config/fish/config.fish`:

```fish
conch build fish --config ~/.config/conch/conch.toml | source
```

**Bash** — in `~/.bashrc`:

```bash
eval "$(conch build bash --config ~/.config/conch/conch.toml)"
```
...
This is an excerpt from `conch build fish --config examples/conch.toml` — your generated file will include every block from your config, in resolved order:
